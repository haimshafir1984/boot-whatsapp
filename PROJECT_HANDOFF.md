...
## עדכון 2026-09-21 (שלב E — C0 תכנון בלבד)

- נכתב `docs/stage-e-design-2026-09-21.md` (סכימה + אינדקסים, שורת scheduler ואינוואריאנטות, מכונת מצבים, מרוצים, conversationState, ספי מדידה, מאמץ ~11–14 ימים). **אין קוד מימוש.** ממתין לאישור C0 והכרעות D1–D7 לפני C1.
- ממצא חדש: `cloneSnapshotForTables` מעתיק את כל snapshot השיחות בכל drain (לא row-tracked) — עלות לינארית שנייה (~4ms ב-1,200 שיחות, ~86ms ב-20,000 על מכונת הפיתוח; `set()` עוד ~5ms/~77ms). המספרים הישנים (110ms/47.5ms/13MB) היסטוריים.
- ממצא: reclaim של `processing` ב-inbox הלקוח מריץ מחדש בשקט (סיכון שליחה כפולה) — התכנון מציע `review` ידני כברירת מחדל.
- נוספו: `scripts/measure-convstate-hotpath.js`, `scripts/measure-convstate-drain.js`, `docs/results-data/convstate-*-2026-09-21.*`.

## עדכון 2026-09-21 (שלב E — C1 + C2, מקומי בלבד)

- C0 אושר (D1–D7: review ידני ללקוח עם lease שפג בתנאי מדידה ב-C6; needs_review לשולח; מסד `flowsbiz_inbox_test` ייעודי; dedupe 30י'/payload 7י'; requeue מאחורי עבודה פעילה; הסרת shadow ב-PG כפוף להוכחות ב-C4).
- **C1:** ההצטברות משוחזרת בבסיס `098b94f`: בייטים ×119.7, סריקה ×2.0 מהפריטים, זמן p95 ×48.9; event-loop delay p99 ~5.2s בפרץ 150 ב-50,000 פריטים. קריטריון משנה (שיפוע backlog בזמן 3.92 מול 5) החמיץ — דווח, סף לא שונה. גיזום: 50,000 completed => 380 בפועל.
- **C2:** `src/inbox/*` (repository אסינכרוני ל-PG, סכימה ב-`inbox_schema_migrations`, scheduler row) — לא מחובר לזמן-ריצה. 21/21 בדיקות PG, 8/8 מוטציות, שער הביצועים עבר (זמן/WAL שטוחים, שורות שנסרקו ≤50, 0 הפרות אינווריאנטות). נתפסו ותוקנו: `clock_timestamp()` volatile (claim סרק את כל השולחים) ו-`= ANY` לדפי review; `enable_seqscan=off` ב-pool ה-inbox.
- שתי ההנחות אומתו: פריט אחד לשולח (בקוד + הרצה על עותק הבסיס); handover חוזר בטוח בתנאי שזהות הפריט נשמרת בלקוח (30 יום) — קריאה בקוד, קצה-לקצה ב-C6.
- שם משתנה מוצע לבעל המערכת: `INBOX_DATABASE_URL` (שער). **עדיין לא מוגדר בקוד — יוגדר ב-C3.**
- פירוט: `docs/stage-e-c1-c2-results-2026-09-21.md`. **C3 ממתין לאישור.**

## עדכון 2026-09-21 (שלב E — C3 חיבור ה-Inboxes, מקומי בלבד)

- **`INBOX_DATABASE_URL` הוגדר בקוד** (`src/inbox/config.ts`): שער עם `INBOX_BACKEND=postgres` דורש אותו (אין מסד אחר); לקוח משתמש ב-`DATABASE_URL` אם לא הוגדר. ברירת מחדל `INBOX_BACKEND=json` — אין שינוי בייצור עד הפעלה מפורשת. נדרש לפיילוט: PostgreSQL לשער (בעל המערכת).
- `enable_seqscan=off` הוסר; שערי C2 עוברים בלעדיו. ראיות C1/C2 בריפו (`docs/results-data/`, `SHA256SUMS-stage-e.txt`); סקריפטי מדידה כותבים לשם כברירת מחדל ורצים אחרי `measure-preflight`.
- שני ה-drainers, קבלות ה-HTTP, פעולות המנהל והכיבוי אסינכרוניים מול `InboxStore` (PG / JSON legacy); 200/202 רק אחרי commit; כשל SQL => 503, בלי fallback; stale => `review`; עמום => `review` + `needs_review`; requeue מאחורי עבודה פעילה. 15/15 בדיקות runtime, 6/6 מוטציות, רגרסיה 70/71 (+1 BLOCKED ידוע).
- לפני C6: `docs/stage-e-c6-comparison-plan-2026-09-21.md` (אין טענת שיפור ב-backlog; 50,000 לא בר-השוואה). תהליכי node "תקועים" = סשני Codex/MCP חיים, לא הריפו.
- פירוט: `docs/stage-e-c3-results-2026-09-21.md`. **C4 ממתין לאישור.**

## עדכון 2026-09-21 (שלב E — C4 conversationState, מקומי בלבד)

- **PostgreSQL בלבד:** שינוי שיחה נשמר כשורה בודדת — בלי snapshot מלא, בלי deep-clone מלא, בלי קובץ shadow; שחזור רק מה-DB. נסגרו **שני** החצאים של העלות (`persist()` ו-`cloneSnapshotForTables` שלא הכיר את הטבלה כ-row-tracked). מצב JSON ללא שינוי.
- שש ההוכחות (T1–T6) עברו על PG אמיתי (7/7), מוטציות 7/7. מדידה בסיס מול חדש, אותו סקריפט: `set()` ב-20,000 שיחות 206ms → 0.08ms; event-loop max 256 → 14ms; קובץ shadow לא נכתב.
- רגרסיה 71/72 (+1 BLOCKED ידוע). מיפוי ה-`return` השקטים: stale (טופל ב-C3) הוא היחיד המהותי; מקרה שולי פתוח בשורה 1447 (תשובה על החלטה שפגה + קמפיין שנמחק).
- הערת פריסה: קבצי `conversation-state.json` קיימים בשרתים נשארים כפי שהם ולא מתעדכנים ב-PG.
- פירוט: `docs/stage-e-c4-results-2026-09-21.md`. **C5 ממתין לאישור.**

## עדכון 2026-09-21 (שלב E — C5 מיגרציה, C6 השוואת מערכת, תיקוני כיבוי ו-D1; מקומי בלבד, ללא commit)

- **C5:** `src/inbox/migration.ts` + `scripts/inbox-migrate.js` (dry-run שאינו משנה דבר; apply עם גיבוי+sha256, בדיקת מקור שקט, batches אידמפוטנטיים, אימות ספירות/digest לפני הפעלה, `.sql-active`, המקור נשמר `.migrated-<ts>`; rollback שמחזיר את המקור בייט-לבייט אם SQL לא עבד, ואחרת מסרב ומחייב export מאומת). 10/10 בדיקות PG. הוראות פריסה: `docs/inbox-migration-runbook-2026-09-21.md` (כולל: `conversation-state.json` בשרתים נשאר ולא מתעדכן במצב PostgreSQL).
- **C6 (תהליכים נפרדים, מכונת פיתוח, ריצה אחת לתא):** burst 100 עם היסטוריה 50,000: בסיס p50 84s / p99 226s, לולאת אירועים בשער 12.7s, 1,592 CLIENT_SKIPPED; חדש p50 1.4s, 0 skipped. sustained 1000 ב-20 דק': בסיס נשאר עם backlog, חדש 0. אין טענת שיפור ב-backlog (ראו `docs/stage-e-c6-comparison-plan-2026-09-21.md`); 50,000 מול 50,000 אינו ברי-השוואה.
- **נמצא ותוקן בבדיקות תקלות:**
  1. **נפילת מסד ה-inbox קורסת את התהליך.** שני מקורות: (א) אין `pool.on('error')` ב-`createInboxPool`; (ב) לקוח שנלקח מה-pool באמצע טרנזקציה ונהרג פולט `error` בלי מאזין (`tx()` ב-`postgresRepository.ts` מטפל עכשיו וקוטע את החיבור). אחרי התיקון: הפסקת מסד 60s ו-180s — lost 0, dup 0, `queued/processing` = 0 בסוף.
  2. **הכיבוי המסודר נתקע ב-`server.close()`** (7 מתוך 11 כיבויים הגיעו ל-forceExit ב-8s, exit 1, ועובדים לא נעצרו) בגלל חיבורי keep-alive שהשער מחזיק. תיקון ב-`src/shutdown.ts`: `closeIdleConnections()` חוזר כל 100ms עד ש-`close()` חוזר (קריאה בודדת לא הספיקה: סוקטים שהיו פעילים ברגע ה-close הופכים סרק אחריו). `graceMs`, סדר השלבים ושאר התקציבים לא שונו. תחת עומס: 12/12 כיבויים exit 0 תוך 0.3–2.1s. בדיקה: `scripts/test-shutdown-idle-connections.js` (5/5).
  3. **ה-harness ב-Windows:** `child.kill('SIGTERM')` הוא הריגה קשיחה — מספרי "SIGTERM" הקודמים (1/9/10 פריטים מופרעים) היו בפועל SIGKILL. ה-harness שולח עכשיו SIGTERM אמיתי דרך IPC ל-handler האמיתי של האפליקציה.
- **D1 — אופציה 1 בוצעה** (`src/messageFlow.ts`): טריגר מדויק חדש ממשתתף שמוחזק בגלל פריט עמום (`source:'inbox'`, בלי `recovery`) משחרר את ה-hold ומתחיל ריצה חדשה, בלי מנהל. hold של מנהל/כשל לא נדרס; הודעות שהוחזקו נזרקות עם לוג מפורש. בדיקה בתוך `test-inbox-runtime-postgres` (18/18), נכשלת כשהקריאה מבוטלת.
- **חשיפה שיורית ידועה (מגבלה מודעת, לא פער שנשכח):** הריגה קשיחה של לקוח (קריסה / OOM / SIGKILL) בחלון צר, כשיש עבודה בתעופה, משאירה עד **~21 פריטים** (נמדד: 21/9/7 בשלוש ריצות) בסטטוס `review(ambiguous_processing)` לבדיקה ידנית, והמשתתפים שלהם לא מקבלים תשובה. למשתתף יש מסלול מילוט: שליחת הטריגר מחדש (אופציה 1). **אין פתרון אוטומטי מעבר לזה** — אופציות 2 (סימון אפקט מדויק) ו-3 (הוכחת המשך בטוח) הוחלט שלא לבצע. החשיפה תלויית-תזמון: 4 הריגות ב-0.7s ו-2 הריגות ב-4s/5s נתנו 0 פריטים; כיבוי מסודר (SIGTERM) אחרי תיקון הכיבוי לא מפריע פריטים. מדידת "עקבות" (כמה מהפריטים העמומים נקיים לחלוטין) **לא הופקה** — לא נוצרו פריטים עמומים באצוות שהורצו (ריצות 2s ו-3s נכשלו בלי תוצאה, לא נבדקו).
- **פתוח:** (1) תשובה להחלטה שפג תוקפה כשהקמפיין נמחק חוזרת בשקט (ה-`return` השקט היחיד שנותר); (2) קבצי `conversation-state.json` ישנים בשרתים; (3) משלוח מייל ההתראות לא נבדק; (4) מיגרציה בייצור — הרשאה נפרדת; (5) הקצאת `INBOX_DATABASE_URL` לשער לפני פיילוט; (6) הסופק (3 שעות) — אחרי commit.

## עדכון 2026-09-22 (שלב E — הסופק 3 שעות, מקומי בלבד)
