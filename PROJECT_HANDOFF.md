...
## עדכון 2026-08-29 - ביצועי PostgreSQL, ניתוב חוצה-לקוחות וסדר משלוח קבצים

קומיטים: `ce7abee`, `0509b6b`, `a329514`.
...
התגלה שהגדרת `contactsProvider` (Google/ידני) היא **הגדרה גלובלית בדשבורד, לא הגדרת קמפיין**, וברירת המחדל לכל התקנת Meta Cloud API חדשה היא `'google'` - **גם אם Google מעולם לא חובר בפועל**. כשזה קורה, המשימות נערמות בתור בשקט (`pending`, לא `failed`) בלי שאף פעם מנסים אותן, כי ה-worker מדלג על משימות Google כל עוד Google לא מחובר. יהודית פיטנס הועברה ידנית למצב `manual` דרך הדשבורד. **לא תוקן קוד עבור ברירת המחדל עצמה** - זה נשאר עניין פתוח לבדוק אצל לקוחות אחרים לפני קמפיינים גדולים (ראו "נקודות תשומת לב" למטה).
...
2. כדאי לבדוק אצל לקוחות אחרים (לא רק יהודית פיטנס) האם `contactsProvider` מוגדר ל-`google` בלי חיבור Google בפועל - זה גורם למשימות שמירת אנשי קשר להיערם בשקט בלי אף פעם להתבצע.
