...
| `7235874` | Record independent verification of the decision-recovery-scale-fix branch |
| `f90fa33` | Fix phoneIndex phone-reassignment ordering bug found by Codex (round 3) |
...
## 9. סבב סקירה שלישי (קודקס) — באג נוסף ב-A.2, תוקן

קודקס מצא **מקרה קצה נוסף שלא כוסה** בתיקון הקודם (§8), עם שחזור מדויק מול ה-build:

> 1. `jidA` נכנס ראשון עם מספר X. 2. `jidB` נכנס אחריו עם מספר Y. 3. `jidA` מתעדכן למספר Y.
> הסריקה הישנה מחזירה `jidA` (נכנס ראשון ל-`this.map`); האינדקס (אחרי התיקון הקודם) החזיר `jidB`.

**האבחנה המדויקת:** הסריקה המקורית לא בודקת "מי הצטרף לטלפון הזה קודם" — היא מריצה
`for (const state of this.map.values())` בסדר ההוספה **הגלובלי** של `this.map`, ובודקת את הטלפון
**הנוכחי** של כל רשומה. כלומר "ראשון" = ראשון בסדר ההוספה הגלובלי מבין הרשומות שהטלפון **הנוכחי**
שלהן תואם — לא ראשון שקיבל את הטלפון הזה ספציפית. התיקון הקודם (§8) תיקן רק את המקרה של רענון-במקום
(אותו טלפון), אבל `reindexPhone` עדיין הוסיף jid בסוף ה-`Set` של הטלפון **החדש** בכל שינוי טלפון —
נכון ל-jid חדש-לגמרי (שבאמת אחרון גם ב-`this.map` הגלובלי), אבל **שגוי** ל-jid קיים שמשנה טלפון,
כי המיקום שלו ב-`this.map` הגלובלי יכול להיות **לפני** jid-ים אחרים שכבר על הטלפון הזה.

**שחזור עצמאי, לפני התיקון:**

```
index= whatsapp:972530000011   (jidB — שגוי)
expected= whatsapp:972530000010@c.us   (jidA — נכון, לפי הסריקה הישנה)
```

**התיקון:** שינוי טלפון על jid **קיים** (לא הוספה חדשה) גורם ל-rebuild מלא של ה-`Set` של הטלפון
**החדש**, מסריקה טרייה של `this.map` בסדר ההוספה הגלובלי שלו — `jid` המשויך-מחדש נכלל בכוח (כי
הרשומה שלו ב-`map` עדיין מחזיקה את הטלפון הישן ברגע שהפונקציה רצה, לפני `map.set`), יחד עם כל רשומה
שהטלפון הנוכחי שלה כבר תואם. זו פעולת O(n), אבל **רק כששינוי טלפון אמיתי קורה** — נדיר, לא הנתיב החם
(שהוא רענון-באותו-טלפון, מטופל כבר כ-no-op). jid חדש-לגמרי ממשיך פשוט להתווסף לסוף ה-`Set` — זה עדיין
נכון, כי jid חדש הוא, מעצם הגדרתו, גם אחרון ב-`this.map` הגלובלי.

**אומת מחדש מול ה-repro המדויק של קודקס** (אחרי התיקון): `index= whatsapp:972530000010@c.us` — תואם.

**בדיקה חדשה** (`test-decision-recovery-scale.js`, תרחיש 3ג) משחזרת בדיוק את התרחיש הזה, כולל בדיקת
עקביות מול ה-oracle לפני ואחרי, ובדיקה שהטלפון הישן כבר לא מפנה לשום מקום. **מוטציה עצמאית**: החזרתי
את הגרסה הנאיבית (append-only) ב-`dist/conversationState.js` המקומפל, הרצתי — 3ג נכשל בדיוק עם
`expected 'whatsapp:972530000010@c.us', actual 'whatsapp:972530000011'` (אותה שגיאה בדיוק כמו
בשחזור הידני), שחזרתי את הגרסה המתוקנת — עבר נקי.

**סבב רגרסיה מלא נוסף** (47 סוויטות, `test-referral-ranking.js` הוחרג מראש כמאושר כבר כלא-קשור):
כולן ירוקות, כולל שלוש הבדיקות של A/B ו-`test-postgres-migration-pilot.js` שנכשל באותה צורה הידועה
(דורש `DATABASE_URL`).

**סטטוס:** שני הסבבים של קודקס (A.2 refresh-in-place, A.2 phone-reassignment) תוקנו ואומתו באופן
עצמאי עם מוטציה בכל אחד. חלק B אושר ע"י קודקס ללא הערות נוספות. ממתין לאישור סופי לפני merge/push.
