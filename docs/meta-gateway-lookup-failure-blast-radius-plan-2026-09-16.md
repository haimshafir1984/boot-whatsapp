...
## מומש (16.9.2026, קומיט `41e164c`)

יושם התיקון המתוקן, בדיוק לפי הכרעות Opus 5, ב-[`routeMetaGatewayInbound`](../src/adminServer.ts#L1738):

1. לולאת הגילוי מפרידה בין `routeListFailures` (כשל ב-route list, עדיין חוסם הכל מיידית, ללא שינוי) לבין `pendingCheckFailedClients` (Map, לא רק מונה — כשל ב-pending check בלבד, לא חוסם רישום מועמדים לטריגר).
2. כשנמצאת התאמת טריגר חד-משמעית: לקוחה שכשלה ב-pending check מקבלת עכשיו קריאת עצירה עיוורת (`meta-clear-pending`) יחד עם לקוחות ה-stale הידועות. אם היא לא מאשרת `cancelled: true` — נזרק ונופל ל-retry הקיים, **בדיוק** כמו לקוחה ידועה-תקועה שנכשלת.
3. בענף ה-fallback (אין טריגר טרי, מסתמכים על pending): `pendingLookupFailures` נזרע כעת מ-`pendingCheckFailedClients.size` האמיתי מלולאת הגילוי, במקום המונה המקומי השבור שלא יכול היה אף פעם לעלות בפועל (זה בדיוק הבאג השלישי, החמור ביותר, שאותר).
4. התרעת מערכת חדשה `meta-gateway-client-blocking-peers-<clientId>` (severity: warning, throttle 30 דק' לכל לקוחה בנפרד) בכל שלושת נקודות החסימה — לפי הדרישה המפורשת לדעת תמיד איזו לקוחה "מפריעה" לאחרות.

**אימות**: build נקי; 5 בדיקות end-to-end חדשות ב-`scripts/test-meta-gateway-lookup-failure-isolation.js` (שרתי HTTP אמיתיים במקום הלקוחות, דרך `startAdminServer` האמיתי — לא mock של לוגיקת הניתוב) שמכסות בדיוק את התרחישים שOpus סימן, כולל **בדיקת מוטציה** שהוכיחה שבדיקה 4 (הבאג הכי חמור) באמת נכשלת כשמבטלים את התיקון; סבב רגרסיה של 22 סוויטות (כולל כל אלו שOpus ציין: meta-campaign-routing, meta-mixed-rollout-routing, meta-cancellation-boundary, meta-switch-inflight, gateway-inbox/reliability, routes-cache, ועוד) — כולן ירוקות.

### מגבלה שיורית שחשוב לדעת — לא נפתרה, ולא הייתה אמורה להיפתר בסבב הזה

התיקון הזה פותר במלואו את מקרה **הכשל החולף** — לקוחה שה-route list שלה עדיין תקף (בתוך TTL של 5 שניות) אבל בדיקת ה-pending הבודדת שלה נכשלה (תקלת רשת רגעית, עומס זמני). במקרה כזה: אין עיכוב כלל, וההודעה ללקוחה הבריאה מנותבת מיד.

**לקוחה שנופלת לגמרי לפרק זמן ממושך (כמו בשחזור המבוקר שביצענו) עדיין תיצור עיכוב דומה למה שנמדד (עד ~37 שניות)** — כי ברגע שגם ה-route list שלה מתיישן (התוקף פג אחרי כ-5 שניות, וה-refresh ברקע נכשל וכן מוחק את הערך השמור), `routeListFailures` נכנס לתמונה באופן עצמאי ובלתי-תלוי בתיקון הזה, וזה עדיין חוסם הכל, בצדק — כי בלי route list אי אפשר לקבוע ambiguity בבטחה. זו לא בעיה בתיקון; זו הכרעת בטיחות מכוונת של Opus, שנשמרה כמו שהיא.

**אם רוצים לצמצם גם את התרחיש הזה** (לקוחה שנופלת לגמרי לזמן ממושך) — יש שיפור נפרד ולא-סקור עדיין: לא למחוק את ה-cache של רשימת הקמפיינים מיד כשרענון ברקע נכשל (`AsyncExpiringCache`, [metaGatewayReliability.ts:150-153](../src/metaGatewayReliability.ts#L150)), אלא להמשיך לשרת את הערך האחרון-הידוע-כתקין למשך זמן ארוך יותר (רשימת טריגרים כמעט לא משתנה משנייה לשנייה). זה **לא מומש בסבב הזה** ודורש סבב אישור נפרד.
