...
(async () => { try {
  ...
  await inbound(storage, transport, flowPhone, 'rank', true); assert(transport.sent.some((item) => item.type === 'text' && item.text.includes('Rank ')), 'my-rank action must render the editable template');
