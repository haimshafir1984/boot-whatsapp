...
- Service נפרד, שנוצר ומוחל תחילה ללא קוד כדי שניתן יהיה לחבר אליו אחסון והגדרות.
...
ההקמה מחילה תחילה Service ריק שאינו מריץ את האפליקציה, כדי שה-Service יהיה יעד תקף לחיבור Volume. לאחר מכן היא מחברת את האחסון והסיסמה, מחברת את הקוד ומבצעת Deploy סופי. כך ה-Deployment הראשון של הבוט אינו עולה ללא `CLIENT_ACCESS_TOKEN`. אין צורך בלחיצה ידנית על `Deploy`. טוקן `RAILWAY_PROJECT_TOKEN` נשאר רק בשירות המנהל ואינו מועבר לשירותי הלקוחות. אם ההקמה נעצרת באמצע, דשבורד המנהל שומר את מזהי משאבי Railway שכבר נוצרו ויכול לנסות שוב בלי ליצור Service או Volume כפולים.
